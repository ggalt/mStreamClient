...
//        onSourceChanged: {
//            myLogger.log("source changed")
//            playlistingForm.setCurrentModel(currentPlayListJSONModel.model)
//        }
        onModelChanged: myLogger.log("model changed")
    }
    ...
    function setSettings() {
        myLogger.log(settingsForm.setServerURL, settingsForm.setPortNumber, settingsForm.setUserName, settingsForm.setPassword )
...
//            myLogger.log("Playlist Click")
    ...
    function updatePlaylist(m_item, typeOfItem, action) { // m_item needs to be the name of an artist, album, playlist or song
        ...
        if(action === "replace") {
            currentPlayList.clearMe()
            myLogger.log("currentPlayList.count:", currentPlayList.count)
            myLogger.log("playlist:", currentPlayList.json)
//            playlistAddAt = 0
        }
    ...
    function loadToPlaylist() {
        myLogger.log("Callee:", arguments.callee.toString())
//        myLogger.log("gettingArtists is:", gettingArtists, "gettingAlbums is:", gettingAlbums, "gettingTitles is:", gettingTitles)
        if( gettingArtists <= 0 && gettingAlbums <= 0 && gettingTitles <= 0) {
            myLogger.log("loading playlist whch has length of:", curPlayLst.count)
            isPlaying = true
            curPlayLst.updateList()
            myLogger.log("Trying again . . . loading playlist whch has length of:", curPlayLst.count)
            ...
            for( var obj in currentPlayList) {
                myLogger.log(obj.metadata["title"])
            }
        }
    }
